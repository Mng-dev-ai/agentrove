...
class UploadCommandAction:
    ...
    async def execute(self, user_id: UUID, file: UploadFile, db: AsyncSession) -> CustomSlashCommandDict:
        try:
            user_settings = cast(
                UserSettings,
        ...
        try:
            command_data = await self._command_service.upload(str(user_id), file, current_commands)
