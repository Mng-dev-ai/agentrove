...
class GetUserUsageAction:
    ...
    async def execute(self, current_user: User) -> UserUsage:
        messages_used = await self._user_service.get_user_daily_message_count(current_user.id)
        messages_remaining = await self._user_service.get_remaining_messages(current_user.id)
