...
class DeleteMcpAction:
    ...
    async def execute(self, user_id: UUID, mcp_name: str, db: AsyncSession) -> McpDeleteResponse:
        ...
        mcp_index = next((i for i, m in enumerate(current_mcps) if m.get("name") == mcp_name), None)
        ...
        if self._user_service.remove_installed_component(user_settings, f"mcp:{mcp_name}"):
